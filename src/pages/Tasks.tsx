...
import { CalendarDays, CheckSquare, Trash2, Edit, Clock, User, Paperclip, ListChecks, Filter, GripVertical, ChevronDown, Save, X, MessageSquare, HelpCircle, LayoutTemplate, ArrowUpDown } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
          ...
          {view === 'list' && (
            <div className="space-y-3">
              {filtered.map((t) => (
                <Card
                  key={t.id}
                  className={`bg-[#0f0f13] border-zinc-800 rounded-xl transition-all ${editing?.id === t.id ? 'ring-2 ring-primary border-transparent' : 'border'}`}
                >
                  {/* Task Header Row */}
                  <div
                    ...
                    onClick={() => {
                      if (editing?.id === t.id) setEditing(null); // Toggle close
                      else setEditing(t); // Toggle open
                    }}
                  >
                    ...
                    <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform ${editing?.id === t.id ? 'rotate-180' : ''}`} />
                  </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div className="flex gap-2">
                              <Input type="date" value={editing.dueDate || ''} onChange={(e) => setEditing({ ...editing, dueDate: e.target.value })} className="flex-1" />
                              <Input type="time" value={editing.dueTime || ''} onChange={(e) => setEditing({ ...editing, dueTime: e.target.value })} className="w-[100px]" />
                            </div>
                            <div className="flex gap-2">
                          ...
                          <AccordionItem value="assignees">
                            ...
                            <AccordionContent className="px-4 pb-4">
                              ...
                              <div className="mt-4 space-y-2">
                                ...
                                <Input value={editing.customerId || ''} onChange={(e) => setEditing({ ...editing, customerId: e.target.value })} placeholder="Link customer (id/name)" className="h-8 text-sm" />
                                <Input value={editing.vehicleId || ''} onChange={(e) => setEditing({ ...editing, vehicleId: e.target.value })} placeholder="Link vehicle (id)" className="h-8 text-sm" />
                                <Input value={editing.workOrderId || ''} onChange={(e) => setEditing({ ...editing, workOrderId: e.target.value })} placeholder="Link work order (id)" className="h-8 text-sm" />
                              </div>
                            </AccordionContent>
                          </AccordionItem>
                        )}
                              ...
                              {editing.checklist.map((c, idx) => (
                                <div key={c.id} className="flex items-center gap-2">
                                  <Checkbox checked={c.done} onCheckedChange={(v) => {
                                    const list = editing.checklist.slice(); list[idx] = { ...c, done: !!v }; setEditing({ ...editing, checklist: list });
                                  }} />
                                  ...
                                  <Button size="icon" variant="ghost" className="h-8 w-8 text-red-400" onClick={() => {
                                    const list = editing.checklist.slice(); list.splice(idx, 1); setEditing({ ...editing, checklist: list });
                                  }}>
                                    <X className="w-3 h-3" />
                                  </Button>
                                </div>
                              ))}
                              <Button size="sm" variant="outline" className="w-full mt-2" onClick={() => setEditing({ ...editing!, checklist: [...editing!.checklist, { id: `chk_${Math.random().toString(36).slice(2, 6)}`, text: '', done: false }] })}>+ Add Item</Button>
                            </div>
                          </AccordionContent>
                        </AccordionItem>
                                ...
                                {editing.attachments.map(a => (
                                  <div key={a.id} className="flex items-center gap-2 text-sm bg-zinc-900 p-2 rounded">
                                    ...
                                    <Button size="icon" variant="ghost" className="h-6 w-6 text-red-400" onClick={() => {
                                      setEditing({ ...editing!, attachments: editing!.attachments.filter(x => x.id !== a.id) });
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex gap-2 pt-2">
                                <Input
                                  ...
                                  onChange={(e) => setEditing({ ...editing!, _newComment: e.target.value } as any)}
                                  className="h-9"
                                />
